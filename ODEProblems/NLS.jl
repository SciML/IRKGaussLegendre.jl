...
function NLSODE!(du, u, p, t)
    ...
    for i = 2:(N-1)
        du[i] =
            p[i] * (q[i]^2 + p[i]^2) -
            (2 * p[i-1]^2 * p[i] - 2 * q[i-1]^2 * p[i] + 4 * p[i-1] * q[i-1] * q[i]) -
            (2 * p[i] * p[i+1]^2 - 2 * p[i] * q[i+1]^2 + 4 * p[i+1] * q[i] * q[i+1])

        du[i+N] =
            -q[i] * (q[i]^2 + p[i]^2) +
            (2 * q[i-1]^2 * q[i] - 2 * p[i-1]^2 * q[i] + 4 * p[i-1] * p[i] * q[i-1]) +
            (2 * q[i] * q[i+1]^2 - 2 * q[i] * p[i+1]^2 + 4 * p[i] * p[i+1] * q[i+1])
    end
    ...
    du[2*N] =
        -q[N] * (q[N]^2 + p[N]^2) +
        (2 * q[N-1]^2 * q[N] - 2 * p[N-1]^2 * q[N] + 4 * p[N-1] * p[N] * q[N-1])
end
