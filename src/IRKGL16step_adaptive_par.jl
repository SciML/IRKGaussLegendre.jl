...
function IRKstep_par_adaptive!(s,j,ttj,uj,ej,prob,dts,coeffs,cache,maxiters,maxtrials,
...
function IRKstep_par_adaptive_Mix!(s,j,ttj,uj,ej,prob,dts,coeffs,cache,maxiters,maxtrials,
		                   initial_interp,abstol,reltol,adaptive,threading,
		...
		@unpack U,Uz,L,Lz,F,Dmin,Eval,DY,rejects,nfcn,lambdas,
	         	Ulow,DU,DF,DL,delta,Fa,Fb,Plow,
	         	normU,lhb,lmu,nrmdigits=cache
		...
		while (!accept && ntrials<maxtrialsj)
        	...
        	while (nit<maxiters && iter)
        		...
        		Threads.@threads for is in 1:s
						Eval[is]=false
                		for k in eachindex(uj)
							DY[is]=abs(Rdigits(U[is][k],nrmdigits[])-Rdigits(Uz[is][k],nrmdigits[]))
                        	if DY[is]>0.
						   		Eval[is]=true
                           		if DY[is]< Dmin[is][k]
                              		Dmin[is][k]=DY[is]
...
#							    	iter=true
                           		...
                           		D0+=1
								if abs(U[is][k]-Uz[is][k])>0. Eval[is]=true end
                       		end
                		end
...
function IRKstepDynODE_par_adaptive!(s,j,ttj,uj,ej,prob,dts,coeffs,cache,maxiters,maxtrials,
		                      initial_interp,abstol,reltol,adaptive,threading)
