...
abstract type IRKAlgorithm{mstep,maxtrials,initial_interp,myoutputs,threading,mixed_precision,low_prec_type,nrmbits}  <: OrdinaryDiffEqAlgorithm end
struct IRKGL16{mstep,maxtrials,initial_interp,myoutputs,threading,mixed_precision,low_prec_type,nrmbits} <: IRKAlgorithm{mstep,maxtrials,initial_interp,myoutputs,threading,mixed_precision,low_prec_type,nrmbits} end
IRKGL16(; mstep=1,maxtrials=3,initial_interp=true,myoutputs=false,threading=false,mixed_precision=false,low_prec_type=Float64,nrmbits=6)=IRKGL16{mstep,maxtrials,initial_interp,myoutputs,threading,mixed_precision,low_prec_type,nrmbits}()

function DiffEqBase.__solve(prob::DiffEqBase.AbstractODEProblem{uType,tType,isinplace},
     alg::IRKGL16{mstep,maxtrials,initial_interp,myoutputs,threading,mixed_precision,low_prec_type,nrmbits},args...;
     dt=0.,
     maxiters=100,
     save_everystep=true,
	 adaptive=true,
	 reltol=1e-6,
	 abstol=1e-6,
     kwargs...) where{uType,tType,isinplace,mstep,maxtrials,initial_interp,myoutputs,threading,mixed_precision,low_prec_type,nrmbits}
   ...
   if (save_everystep==false)
    ...
    if (mixed_precision==true && typeof(prob.f)<:ODEFunction)
		...
		cache=tcacheMix{uType,uiType,uLowType,low_prec_type}(U1,U2,U3,U4,U5,U6,
		             ...
		             U11,U12,U13,U14,U15,U16,U17,Plow,
					 fill(zero(low_prec_type),s),lhb,lmu,nrmdig)

		@unpack U,Uz,L,Lz,F,Dmin,Eval,DY,rejects,nfcn,lambdas,
		        Ulow,DU,DF,DL,delta,Fa,Fb,Plow,normU,lhb,lmu,nrmdigits=cache

	 else
		cache=tcache{uType,uiType,uLowType,low_prec_type}(U1,U2,U3,U4,U5,U6,
	 				 fill(true,s),fill(zero(uiType),s),
					 [0],[0,0],fill(zero(uiType),2),nrmdig)
   ...
   if (threading==true && Threads.nthreads()>1)
    ...
    while cont
        ...
        for i in 1:m
		  j+=1
		  k+=1
   	      (status,it) = IRKStep_par!(s,j,tj,uj,ej,prob,dts,coeffs,cache,maxiters,
       ...
       if (save_everystep==true) || (cont==false)
		...
		while cont
	        ...
	        for i in 1:m
			  j+=1
			  k+=1
	   	      (status,it) = IRKStep_seq!(s,j,tj,uj,ej,prob,dts,coeffs,cache,maxiters,
		                                 maxtrials,initial_interp,Tabstol,Treltol,adaptive,
								     	 threading,mixed_precision,low_prec_type)
...
function IRKStep_seq!(s,j,tj,uj,ej,prob,dts,coeffs,cache,maxiters,maxtrials,
...
function IRKStep_par!(s,j,tj,uj,ej,prob,dts,coeffs,cache,maxiters,maxtrials,
  		               initial_interp,abstol,reltol,adaptive,threading,
					   mixed_precision,low_prec_type)
   ...
   if (typeof(prob.f)<:ODEFunction)
   		...
   		if (adaptive==true)
      		if (mixed_precision==true)
	    		(status,it)= IRKstep_par_adaptive_Mix!(s,j,tj,uj,ej,prob,dts,coeffs,cache,maxiters,
 		     	             maxtrials,initial_interp,abstol,reltol,adaptive,
						     threading, mixed_precision,low_prec_type)
	  		else
	    		(status,it)= IRKstep_par_adaptive!(s,j,tj,uj,ej,prob,dts,coeffs,cache,maxiters,
   		     	             maxtrials,initial_interp,abstol,reltol,adaptive,
							 threading)
      		end
       else
      	   if (mixed_precision==true)
	            (status,it)= IRKstep_par_fixed_Mix!(s,j,tj,uj,ej,prob,dts,coeffs,cache,maxiters,
 			                 initial_interp,abstol,reltol,adaptive,threading,
				   			 mixed_precision,low_prec_type)
	       else
		        (status,it)= IRKstep_par_fixed!(s,j,tj,uj,ej,prob,dts,coeffs,cache,maxiters,
				     		 initial_interp,abstol,reltol,adaptive,
							 threading)
	      end
      end
	   ...
	   if (adaptive==true)
	       (status,it)= IRKstepDynODE_par_adaptive!(s,j,tj,uj,ej,prob,dts,coeffs,cache,maxiters,
				        maxtrials,initial_interp,abstol,reltol,adaptive,threading)
	   else
		 (status,it)= IRKstepDynODE_par_fixed!(s,j,tj,uj,ej,prob,dts,coeffs,cache,maxiters,
					   initial_interp,abstol,reltol,adaptive, threading)
	   end
