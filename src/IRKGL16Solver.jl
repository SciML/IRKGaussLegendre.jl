    ...
    if (typeof(prob.f) <: ODEFunction)
        if (adaptive == true)
            if (mixed_precision == true)
                (status, it) = IRKstep_adaptive_Mix!(s, j, tj, uj, ej, prob, dts, coeffs,
                                                     cache, maxiters,
                                                     maxtrials, initial_interp, abstol,
                                                     reltol, adaptive,
                                                     threading, mixed_precision,
                                                     low_prec_type)
            else
                (status, it) = IRKstep_adaptive!(s, j, tj, uj, ej, prob, dts, coeffs, cache,
                                                 maxiters,
                                                 maxtrials, initial_interp, abstol, reltol,
                                                 adaptive,
                                                 threading)
            end
        else
            if (mixed_precision == true)
                (status, it) = IRKstep_fixed_Mix!(s, j, tj, uj, ej, prob, dts, coeffs,
                                                  cache, maxiters,
                                                  initial_interp, abstol, reltol, adaptive,
                                                  threading,
                                                  mixed_precision, low_prec_type)
            else
                (status, it) = IRKstep_fixed!(s, j, tj, uj, ej, prob, dts, coeffs, cache,
                                              maxiters,
                                              initial_interp, abstol, reltol, adaptive,
                                              threading)
            end
        ...
        if (adaptive == true)
            (status, it) = IRKstepDynODE_adaptive!(s, j, tj, uj, ej, prob, dts, coeffs,
                                                   cache, maxiters,
                                                   maxtrials, initial_interp, abstol,
                                                   reltol, adaptive, threading)
        else
            (status, it) = IRKstepDynODE_fixed!(s, j, tj, uj, ej, prob, dts, coeffs, cache,
                                                maxiters,
                                                initial_interp, abstol, reltol, adaptive,
                                                threading)
        end
...
function IRKstep_adaptive_Mix!(s, j, ttj, uj, ej, prob, dts, coeffs, cache, maxiters,
                               maxtrials,
                               initial_interp, abstol, reltol, adaptive, threading,
                               mixed_precision, low_prec_type)
    @unpack mu, hc, hb, nu, alpha = coeffs
...
function IRKstepDynODE_adaptive!(s, j, ttj, uj, ej, prob, dts, coeffs, cache, maxiters,
                                 maxtrials,
                                 initial_interp, abstol, reltol, adaptive, threading)
    @unpack mu, hc, hb, nu, alpha = coeffs
    @unpack tspan, p = prob
    ...
    while (!accept && ntrials < maxtrialsj)
        ...
        while (nit < maxiters && iter)
            ...
            @inbounds begin for is in 1:s
                Uz[is].x[1] .= U[is].x[1]
                DiffEqBase.@.. U[is].x[1] = uj.x[1] + (ej.x[1] + mu[is, 1] * L[1].x[1] +
                                             mu[is, 2] * L[2].x[1] +
                                             mu[is, 3] * L[3].x[1] + mu[is, 4] * L[4].x[1] +
                                             mu[is, 5] * L[5].x[1] + mu[is, 6] * L[6].x[1] +
                                             mu[is, 7] * L[7].x[1] + mu[is, 8] * L[8].x[1])
            ...
            @inbounds begin for is in 1:s
                Uz[is].x[2] .= U[is].x[2]
                DiffEqBase.@.. U[is].x[2] = uj.x[2] + (ej.x[2] + mu[is, 1] * L[1].x[2] +
                                             mu[is, 2] * L[2].x[2] +
                                             mu[is, 3] * L[3].x[2] + mu[is, 4] * L[4].x[2] +
                                             mu[is, 5] * L[5].x[2] + mu[is, 6] * L[6].x[2] +
                                             mu[is, 7] * L[7].x[2] + mu[is, 8] * L[8].x[2])
