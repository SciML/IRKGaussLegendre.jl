...
function MuCoefficients!(mu,T)
          ...
          mu[1,1]=convert(T,parse(BigFloat,"0.5"))
          mu[1,2]=convert(T,parse(BigFloat,"-8.1894963105581497136508164735930892e-02"))
          mu[1,3]=convert(T,parse(BigFloat,"+4.0042703777945052339969045601553216e-02"))
          mu[1,4]=convert(T,parse(BigFloat,"2.4721345803200374868044581645082913e-02"))
          mu[1,5]=convert(T,parse(BigFloat,"+1.6976173236371093026881708761116103e-02"))
          mu[1,6]=convert(T,parse(BigFloat,"-1.2225914113298206053942531721943548e-02"))
          mu[1,7]=convert(T,parse(BigFloat,"8.7485667691973688117766530139122287e-03"))
          mu[1,8]=convert(T,parse(BigFloat,"-5.4828082532158826793409353214950813e-03"))

          mu[2,1]=convert(T,parse(BigFloat,"+1.0818949631055814971365081647359309e+00"))
          mu[2,2]=convert(T,parse(BigFloat,"0.5"))
          mu[2,3]=convert(T,parse(BigFloat,"-8.6958924300832723329070964616248016e-02"))
          mu[2,4]=convert(T,parse(BigFloat,"+4.4941126302625688139830943466131237e-02"))
          mu[2,5]=convert(T,parse(BigFloat,"-2.8759775474749310978230557041068536e-02"))
          mu[2,6]=convert(T,parse(BigFloat,"+2.0017127636408709173710417097426705e-02"))
          mu[2,7]=convert(T,parse(BigFloat,"-1.4074355889166929145973516652599415e-02"))
          mu[2,8]=convert(T,parse(BigFloat,"+8.7485667691973688117766530139122287e-03"))

          mu[3,1]=convert(T,parse(BigFloat,"+9.5995729622205494766003095439844678e-01"))
          mu[3,2]=convert(T,parse(BigFloat,"+1.0869589243008327233290709646162480e+00"))
          mu[3,3]=convert(T,parse(BigFloat,"0.5"))
          mu[3,4]=convert(T,parse(BigFloat,"-8.8093838732308313442213871391320316e-02"))
          mu[3,5]=convert(T,parse(BigFloat,"+4.6165464814800034116730885592456977e-02"))
          mu[3,6]=convert(T,parse(BigFloat,"-2.9603873064977937463012598212122325e-02"))
          mu[3,7]=convert(T,parse(BigFloat,"+2.0017127636408709173710417097426705e-02"))
          mu[3,8]=convert(T,parse(BigFloat,"-1.2225914113298206053942531721943548e-02"))

          mu[4,1]=convert(T,parse(BigFloat,"+1.0247213458032003748680445816450829e+00"))
          mu[4,2]=convert(T,parse(BigFloat,"+9.5505887369737431186016905653386876e-01"))
          mu[4,3]=convert(T,parse(BigFloat,"+1.0880938387323083134422138713913203e+00"))
          mu[4,4]=convert(T,parse(BigFloat,"0.5"))
          mu[4,5]=convert(T,parse(BigFloat,"-8.8347161109827784250707380600804499e-02"))
          mu[4,6]=convert(T,parse(BigFloat,"+4.6165464814800034116730885592456977e-02"))
          mu[4,7]=convert(T,parse(BigFloat,"-2.8759775474749310978230557041068536e-02"))
          mu[4,8]=convert(T,parse(BigFloat,"+1.6976173236371093026881708761116103e-02"))

          mu[5,1]=convert(T,parse(BigFloat,"+9.8302382676362890697311829123888390e-01"))
          mu[5,2]=convert(T,parse(BigFloat,"+1.0287597754747493109782305570410685e+00"))
          mu[5,3]=convert(T,parse(BigFloat,"+9.5383453518519996588326911440754302e-01"))
          mu[5,4]=convert(T,parse(BigFloat,"+1.0883471611098277842507073806008045e+00"))
          mu[5,5]=convert(T,parse(BigFloat,"0.5"))
          mu[5,6]=convert(T,parse(BigFloat,"-8.8093838732308313442213871391320316e-02"))
          mu[5,7]=convert(T,parse(BigFloat,"+4.4941126302625688139830943466131237e-02"))
          mu[5,8]=convert(T,parse(BigFloat,"-2.4721345803200374868044581645082913e-02"))

          mu[6,1]=convert(T,parse(BigFloat,"+1.0122259141132982060539425317219435e+00"))
          mu[6,2]=convert(T,parse(BigFloat,"+9.7998287236359129082628958290257329e-01"))
          mu[6,3]=convert(T,parse(BigFloat,"+1.0296038730649779374630125982121223e+00"))
          mu[6,4]=convert(T,parse(BigFloat,"+9.5383453518519996588326911440754302e-01"))
          mu[6,5]=convert(T,parse(BigFloat,"+1.0880938387323083134422138713913203e+00"))
          mu[6,6]=convert(T,parse(BigFloat,"0.5"))
          mu[6,7]=convert(T,parse(BigFloat,"-8.6958924300832723329070964616248016e-02"))
          mu[6,8]=convert(T,parse(BigFloat,"+4.0042703777945052339969045601553216e-02"))

          mu[7,1]=convert(T,parse(BigFloat,"+9.9125143323080263118822334698608777e-01"))
          mu[7,2]=convert(T,parse(BigFloat,"+1.0140743558891669291459735166525994e+00"))
          mu[7,3]=convert(T,parse(BigFloat,"+9.7998287236359129082628958290257329e-01"))
          mu[7,4]=convert(T,parse(BigFloat,"+1.0287597754747493109782305570410685e+00"))
          mu[7,5]=convert(T,parse(BigFloat,"+9.5505887369737431186016905653386876e-01"))
          mu[7,6]=convert(T,parse(BigFloat,"+1.0869589243008327233290709646162480e+00"))
          mu[7,7]=convert(T,parse(BigFloat,"0.5"))
          mu[7,8]=convert(T,parse(BigFloat,"-8.1894963105581497136508164735930892e-02"))

          mu[8,1]=convert(T,parse(BigFloat,"+1.0054828082532158826793409353214951e+00"))
          mu[8,2]=convert(T,parse(BigFloat,"+9.9125143323080263118822334698608777e-01"))
          mu[8,3]=convert(T,parse(BigFloat,"+1.0122259141132982060539425317219435e+00"))
          mu[8,4]=convert(T,parse(BigFloat,"+9.8302382676362890697311829123888390e-01"))
          mu[8,5]=convert(T,parse(BigFloat,"+1.0247213458032003748680445816450829e+00"))
          mu[8,6]=convert(T,parse(BigFloat,"+9.5995729622205494766003095439844678e-01"))
          mu[8,7]=convert(T,parse(BigFloat,"+1.0818949631055814971365081647359309e+00"))
          mu[8,8]=convert(T,parse(BigFloat,"0.5"))
...
function HCoefficients!(mu,hc,hb,nu,h::Type{<:CompiledFloats},
                                    hprev::Type{<:CompiledFloats})
...
function EstimateCoeffs!(beta, T::Type{<:CompiledFloats})
...
function EstimateCoeffs2!(beta2, T::Type{<:CompiledFloats})
